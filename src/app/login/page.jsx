...
export default function LoginPage() {
  ...
  // ✅ Mengambil fungsi login langsung dari store
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // ✅ PERBAIKAN: Validasi hanya user, token bisa kosong (untuk cookie-based auth)
      if (data.user) {
        // Token bisa dari response body atau null (Sanctum cookie-based)
        const authToken = data.token || data.access_token || null;

        // Simpan ke Zustand store
        login(data.user, authToken);

        // Tunggu sebentar agar Zustand persist ke localStorage
        await new Promise(resolve => setTimeout(resolve, 100));

        // Debug: Verifikasi localStorage
        console.log('[LOGIN] Saved to localStorage:', localStorage.getItem('squadup-storage'));
        ...
        // Gunakan router.push untuk navigasi client-side
        setTimeout(() => {
          router.push('/lobby');
        }, 1000);
      } else {
        // Response tidak memiliki user
        console.error('[LOGIN] Invalid response - no user:', data);
        toast.dismiss(loadingToast);
